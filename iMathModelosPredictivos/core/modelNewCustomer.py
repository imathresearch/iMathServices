...
""" Implements the class ModelNewCustomer, a sub class of the class Model

In this class we have two approximations:
- A simple model that tries to distinguish between 3 classes (arpu bajo, arpu medio and arpu alto).
- The combination of 3 binary models. 
    - One binary model that distinguishes between belonging to arpu bajo class or not
    - One binary model that distinguishes between belonging to arpu medio class or not
    - One binary model that distinguishes between belonging to arpu alto class or not
...
class ModelNewCustomer(Model):
    ...
    def __init__(self, dataFile, classifierType=None):
        super(ModelNewCustomer,self).__init__(dataFile, classifierType)
    ...
    def createModel(self, dataFile, classifierType):
        ...
        try:        
            ...
            quality = self.accuracyRelaxedProb();
            ...
            if self.classiferClass == DecisionTreeClassifier:
                ...
                #Create binary model for bajo
                print type(self.YData)
                YData_copy = np.copy(self.YData)
                for index in range(len(self.YData)):
                    if self.YData[index] == 0:
                        self.YData[index] = 1
                    else:
                        self.YData[index] = 0
                self._fit(); #for DT
                self.binary_models['bajo'] = self.model
                bajo_class_prediction = KFold(self.XData, self.YData, self.classiferClass)
                bajo_prob_prediction = KFoldProb(self.XData, self.YData,  2, self.classiferClass)
                
                #Create binary model for medio
                self.YData = np.copy(YData_copy)
                for index in range(len(self.YData)):
                    if self.YData[index] != 1:
                        self.YData[index] = 0
                self._fit(); #for DT
                self.binary_models['medio'] = self.model
                medio_class_prediction = KFold(self.XData, self.YData, self.classiferClass)
                medio_prob_prediction = KFoldProb(self.XData, self.YData,  2, self.classiferClass)

                #Create binary model for alto
                self.YData = np.copy(YData_copy)
                for index in range(len(self.YData)):
                    if self.YData[index] == 2:
                        self.YData[index] = 1
                    else:
                        self.YData[index] = 0
                self._fit(); #for DT
                self.binary_models['alto'] = self.model
                alto_class_prediction = KFold(self.XData, self.YData, self.classiferClass)
                alto_prob_prediction = KFoldProb(self.XData, self.YData,  2, self.classiferClass)                
                
                tuple_class_prediction = zip(bajo_class_prediction, medio_class_prediction,alto_class_prediction)
                tuple_prob_prediction = zip(bajo_prob_prediction[:,1], medio_prob_prediction[:,1],alto_prob_prediction[:,1])
                
                self.YData = np.copy(YData_copy)
                
                final_prediction = self.getFinalPrediction(tuple_class_prediction, tuple_prob_prediction)
                quality = self.accuracyPercentageBinaryModels(final_prediction)
                ...
                for index in range(len(self.YData)):
                    if self.YData[index] == 0:
                        ...
                        self.YData[index] = 0
                self._fit(probability=True); #for DT
                self.binary_models['bajo'] = self.model
                bajo_class_prediction = KFold(self.XData, self.YData, self.classiferClass)
                bajo_prob_prediction = KFoldProb(self.XData, self.YData, 2, self.classiferClass)
                ...
                for index in range(len(self.YData)):
                    if self.YData[index] == 0:
                        ...
                        self.YData[index] = 0
                self._fit(n_estimators=10); #for DT
                self.binary_models['bajo'] = self.model
                bajo_class_prediction = KFold(self.XData, self.YData, self.classiferClass)
                bajo_prob_prediction = KFoldProb(self.XData, self.YData,  2, self.classiferClass)
    ...
    def predictModel(self, dataFile, outputFile):
        ...
        try:        
            ...
            '''
            #CODE FOR BINARY MODELS
            #Test binary model for bajo
            self.model = self.binary_models['bajo']
            bajo_class_prediction = self._predict(); 
            bajo_class_prediction[bajo_class_prediction < 1 ] = 0
            bajo_prob_prediction = self._predictProb()
            
            #Test binary model for medio
            self.model = self.binary_models['medio']
            medio_class_prediction = self._predict(); 
            medio_class_prediction[medio_class_prediction < 1 ] = 0
            medio_prob_prediction = self._predictProb()
            
            #Test binary model for alto
            self.model = self.binary_models['alto']
            alto_class_prediction = self._predict(); 
            alto_class_prediction[alto_class_prediction < 1 ] = 0
            alto_prob_prediction = self._predictProb()
            
            tuple_class_prediction = zip(bajo_class_prediction, medio_class_prediction,alto_class_prediction)
            tuple_prob_prediction = zip(bajo_prob_prediction[:,1], medio_prob_prediction[:,1],alto_prob_prediction[:,1])
                                
            final_prediction = self.getFinalPrediction(tuple_class_prediction, tuple_prob_prediction)
            self.__generatePredictionFileBinaryModels(outputFile, ID, final_prediction, tuple_class_prediction, tuple_prob_prediction)
    ...
    def testModel(self, dataFile, outputFile):
        ...
        try:        
            ...
            # Compute confusion matrix
            cm = confusion_matrix(self.YData, final_prediction)
            self.__generateTestFileBinaryModels(outputFile, self.YData, final_prediction, tuple_class_prediction, tuple_prob_prediction, cm)
            print "[iMathResearch] Resultado del testing del modelo guardado en " + outputFile
            '''
    ...
    def _readMetaDataFile(self):
        ...
        """
        lines = [line.strip() for line in open(CONS.MODEL_FILE_METADATA_NEWCUSTOMER)]
        count = 0;
    ...
    def __generatePredictionFile(self, outputFile, ID, prediction, predictionProb):
        ...
        """
        content = []
        header = ['ID subscripcion', 'Clase predicha', "----"]    
        header.append("Prob CL-bajo")
        header.append("Prob CL-medio")
        header.append("Prob CL-alto")
        content.append(header)
